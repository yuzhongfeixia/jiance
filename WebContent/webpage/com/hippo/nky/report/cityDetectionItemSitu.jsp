...
<!-- 							<a class="btn btngroup_usual" onclick="exportExcelByCustom('report.LivestockPlantSituationService.exportDistractMonitor','','searchConditionForm')">导出Excel<i class="icon-share"></i></a> -->
						...
						<table class="table table-striped table-bordered table-hover" style="margin-top: 5px;" id="staticts_table">
							<thead>
								<tr>
									...
									<th class="center hidden-480" >市别</th>
								</tr>
							</thead>
						</table>					
					</div>
	...
	<script>
		...
		function getDataTable(projectCode) {
			$.ajax({
				async: false,
				type : 'POST',
				url : 'livestockSituationController.do?getCityDetectionItemSitu',
				data: {'projectCode':projectCode,'industryCode':industryCode},
				success : function(data) {
					var dataJson = eval('(' + data + ')');
					if(dataJson.success){
						var htmls = dataJson.attributes.htmls;
						var headerSize = dataJson.attributes.headerSize;
						drewTableCells(htmls, headerSize);
					}
				}
			});
		}	
		
		function drewTableCells(htmls, headerSize){
			if (htmls == "") {
	 			var htmls = '<thead><tr>';
	 			htmls += '<th class="center hidden-480">序号</th>';
	 			htmls += '<th class="center hidden-480">市别</th>';
	 			htmls += '</tr></thead>';
			}
			...
			// 设定table的单元格数
			$('#staticts_table').css({"width" : (200 * headerSize + 1000) + 'px'});
...
function doExportExcel(){
	...
	exportExcelByCustom('report.LivestockPlantSituationService.getCommonDetectionItemSituExport','','searchConditionForm');
}
</script> 
</body>
</html>
