...
<script>
var dataTable = null;
dataTable = $('#diffVarietyOverStanderd_table').dataTable({
	"bAutoWidth": true,
	"bPaginate" : false,
...
function drewTableCells(htmls){	
	...
	// 设置拼接的字符串到table中
	$("#diffVarietyOverStanderd_table").html(htmls);
...
$('#statisticsBtn').on('click', function(){
	...
	$.ajax({
		async: false,
		type : 'POST',
		url : 'livestockSituationController.do?getDiffVarietyOverStanderd',
...
function doExportExcel(){
	...
	exportExcelByCustom('report.LivestockPlantSituationService.exportDvos','','searchConditionForm');
}
</script>
...
<!-- 										</select> -->
					...
					<div class="clearfix">
						<div class="pull-right">
							...
							<a class="btn btngroup_usual" onclick="exportExcelByCustom('report.LivestockPlantSituationService.exportDvos','','searchConditionForm')">导出Excel<i class="icon-share"></i></a>
						</div>
					</div>
					<table class="table table-striped table-bordered table-hover" id="diffVarietyOverStanderd_table" style="margin-top: 5px;">
						<thead>
							<tr>
