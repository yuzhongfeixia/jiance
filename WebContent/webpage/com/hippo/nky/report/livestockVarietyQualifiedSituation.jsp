...
<script>
var dataTable = null;
dataTable = $('#livestockVarietyQualifiedSituation_table').dataTable({
	"bAutoWidth": true,
	"bPaginate" : false,
...
function drewTableCells(htmls){	
	...
	// 设置拼接的字符串到table中
	$("#livestockVarietyQualifiedSituation_table").html(htmls);
...
$('#statisticsBtn').on('click', function(){
	...
	$.ajax({
		async: false,
		type : 'POST',
		url : 'livestockSituationController.do?getLivestockVarietyQualifiedSituation',
...
function doExportExcel(){
	...
	exportExcelByCustom('report.LivestockPlantSituationService.exportLvqs','','searchConditionForm');
}
</script>
...
<!-- 										</select> -->
					...
					<table class="table table-striped table-bordered table-hover" id="livestockVarietyQualifiedSituation_table" style="margin-top: 5px;">
						<thead>
							<tr>
