...
<script>
var dataTable = null;
dataTable = $('#monitoringLinkLivestockQualifiedSituation_table').dataTable({
	"bAutoWidth": true,
	"bPaginate" : false,
...
function drewTableCells(htmls){	
	...
	// 设置拼接的字符串到table中
	$("#monitoringLinkLivestockQualifiedSituation_table").html(htmls);
...
$('#statisticsBtn').on('click', function(){
	...
	$.ajax({
		async: false,
		type : 'POST',
		url : 'livestockSituationController.do?getMonitoringLinkLivestockQualifiedSituation',
...
function doExportExcel(){
	...
	exportExcelByCustom('report.LivestockPlantSituationService.exportMLqs','','searchConditionForm')
}
</script>
...
<!-- 										</select> -->
					...
					<table class="table table-striped table-bordered table-hover" id="monitoringLinkLivestockQualifiedSituation_table" style="margin-top: 5px;">
						<thead>
							<tr>
								<th class="center hidden-480" >畜产品种类</th>
