...
$('#statisticsBtn').on('click', function(){
	...
	$.ajax({
		type : "POST",
		url : "livestockSituationController.do?totleSituationStatistics&rand=" + Math.random(),
...
$('#exportWord').bind('click',function(){
	...
	if(isEmpty(projectCode)){
		...
		$(this).attr("href","livestockSituationController.do?exportWord&rand="+Math.random()+"&projectLevel="+projectLevel+"&year="+year+"&projectCode="+projectCode+"&projectName="+encodeURI(encodeURI(projectName)));
	}
});
...
<!-- 									</select> -->
					...
					<div style="margin: 10px 30px 0 10px; font-size: 16pt; line-height: 40px; display: none;" id="showDiv">
						总体情况：<span style="color: red;" id="showYear"></span>年
							     ...
							     <span style="color: red;" id="showCity"></span>个城市
