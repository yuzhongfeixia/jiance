...
<!-- 									</select> -->
						...
						<input type="hidden" id="flg" name="flg" value="4">
					</form>
					</div>
					<div class="clearfix" >
						<div class="pull-right">
							<a class="btn btngroup_seach" id="statisticsBtn"><i class="icon-search"></i>统计</a>  
<!-- 							<a class="btn btngroup_usual" onclick="exportExcelByCustom('report.PlantSituationService.exportCityOF','','searchConditionForm')">导出Excel<i class="icon-share"></i></a> -->
							<a class="btn btngroup_usual" onclick="exportExcelByCustom('report.LivestockPlantSituationService.getCommonLiveStockExport','','searchConditionForm')">导出Excel<i class="icon-share"></i></a>
						</div>
					</div>
	...
	<script>
		...
		function getDataTable(projectCode) {
			$.ajax({
				async: false,
				type : 'POST',
				url : 'livestockSituationController.do?getSampleOverProof',
...
// 			var htmls = '<thead><tr>';
// 			htmls += '<th class="center hidden-480" rowspan="2" style="vertical-align: middle !important"></th>';
// 			htmls += '<th class="center hidden-480" rowspan="2" style="vertical-align: middle !important">抽样总数</th>';
// 			htmls += '<th class="center hidden-480" rowspan="2" style="vertical-align: middle !important">超标数</th>';
// 			htmls += '<th class="center hidden-480" rowspan="2" style="vertical-align: middle !important">超标率</th>';
...
// 				htmls+="<tr><td>总计</td>";
// 				htmls += '<td>'+ getViewData(totalList[totalList.length-1].TOTALSUM,'s') +'</td>';
// 				htmls += '<td>'+ getViewData(totalList[totalList.length-1].ISQUALSUM,'s') +'</td>';
// 				htmls += '<td>'+ getViewData(totalList[totalList.length-1].PCTSUM,'p') +'</td>';
// 				if(activityList != null){
// 					//key值为Map的键值  
// 		            $.each(activityList, function(key, value) {
