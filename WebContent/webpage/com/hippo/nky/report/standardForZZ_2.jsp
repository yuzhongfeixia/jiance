...
<script>
var dataTable = null;
dataTable = $('#standardForZZ_2_table').dataTable({
	"bAutoWidth": true,
	"bPaginate" : false,
...
function drewTableCells(htmls){	
	...
	// 设置拼接的字符串到table中
	$("#standardForZZ_2_table").html(htmls);
...
$('#statisticsBtn').on('click', function(){
	...
	$.ajax({
		async: false,
		type : 'POST',
		url : 'plantSituationController.do?getSuperMarketOverProof',
...
function exportExcel_pcosd(){
	...
	exportExcelByCustom('report.PlantSituationService.exportPcos','','searchConditionForm')
}
</script>
...
<!-- 										</select> -->
					...
					<table class="table table-striped table-bordered table-hover" id="standardForZZ_2_table" style="margin-top: 5px;">
						<thead>
							<tr>
