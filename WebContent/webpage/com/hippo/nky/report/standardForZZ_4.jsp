...
var dataTable = null;
//注册datatable组件
dataTable = $('#standardForZZ_4_table').dataTable({
	"bAutoWidth": true,
	"bPaginate" : false,
...
function drewTableCells(list){	
	$("#standardForZZ_4_table").html("");
	var htmls = '<thead><tr>';
	htmls += '<th class="center hidden-480" >来原地</th>';
	htmls += '<th class="center hidden-480" >来源地数量</th>';
	...
	if(list != null && list.length > 0){
		var sumUnitCount = 0;
		var sumSamplingCount = 0;
		var sumOverStanderdCount = 0;
		var sumDetetionCount = 0;
		for(var i = 0; i < list.length; i++){
			htmls += '<tr>';
			htmls += '<td>'+list[i].monitoringLink+'</td>';
			htmls += '<td>'+list[i].unitCount+'</td>';
			htmls += '<td>'+list[i].samplingCount+'</td>';
			htmls += '<td>'+list[i].overStanderdCount+'</td>';
			htmls += '<td>'+list[i].overStanderdRate+'</td>';
			htmls += '<td>'+list[i].detetionCount+'</td>';
			htmls += '<td>'+list[i].detetionRate+'</td>';
			htmls += '</tr>';
			
			sumUnitCount += list[i].unitCount;
			sumSamplingCount += list[i].samplingCount;
			sumOverStanderdCount += list[i].overStanderdCount;
			sumDetetionCount += list[i].detetionCount;
		}
		if (list.length >= 2) {
			htmls += '<tr>';
			htmls += '<td>合计</td>';
			htmls += '<td>'+sumUnitCount+'</td>';
			htmls += '<td>'+sumSamplingCount+'</td>';
			htmls += '<td>'+sumOverStanderdCount+'</td>';
			htmls += '<td>'+xround(sumOverStanderdCount,sumSamplingCount)+'%'+'</td>';
			htmls += '<td>'+sumDetetionCount+'</td>';
			htmls += '<td>'+xround(sumDetetionCount,sumSamplingCount)+'%'+'</td>';
			htmls += '</tr>';
		}
	} 
	htmls += '</tbody>';
	...
	// 设置拼接的字符串到table中
	$("#standardForZZ_4_table").html(htmls);
...
function xround(x,y){
	return num = new Number(x* Math.pow(10, 1)/y* Math.pow(10, 1)).toFixed(1);
}
...
$('#statisticsBtn').on('click', function(){
	...
	$.ajax({
		async: false,
		type : 'POST',
		url : 'plantSituationController.do?getVegetablesMonitoringLinkOverStanderd',
		data: {'projectCode':projectCode},
		success : function(data) {
			var dataJson = eval('(' + data + ')');
			if(dataJson.success){
				// 总抽样数统计列表取得
				var vegetablesSituationList = dataJson.attributes.vegetablesSituationList;
				drewTableCells(vegetablesSituationList);
			}
		}
	});
});
...
function doExportExcel(){
	...
	exportExcelByCustom('report.PlantSituationService.exportMlos','','searchConditionForm');
}
...
<!-- 							<a class="btn btngroup_usual" onclick="exportExcel('getVegetablesMonitoringLinkOverStanderd','','searchConditionForm')">导出Excel<i class="icon-share"></i></a> -->
							<a class="btn btngroup_usual" onclick="doExportExcel()">导出Excel<i class="icon-share"></i></a>
						</div>
					</div>
					<table class="table table-striped table-bordered table-hover" id="standardForZZ_4_table" style="margin-top: 5px;">
						<thead>
							<tr>
								<th class="center hidden-480" >来原地</th>
								<th class="center hidden-480" >来源地数量</th>
								<th class="center hidden-480" >抽样数</th>
