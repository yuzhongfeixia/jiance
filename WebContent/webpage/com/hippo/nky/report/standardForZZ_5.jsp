...
<script>
var dataTable = null;
dataTable = $('#standardForZZ_5_table').dataTable({
	"bAutoWidth": true,
	"bPaginate" : false,
...
function drewTableCells(list){	
	$("#standardForZZ_5_table").html("");
	var htmls = '<thead><tr>';
	htmls += '<th class="center hidden-480" >类别</th>';
	...
	// 设置拼接的字符串到table中
	$("#standardForZZ_5_table").html(htmls);
...
$('#statisticsBtn').on('click', function(){
	...
	$.ajax({
		async: false,
		type : 'POST',
		url : 'plantSituationController.do?getVegetablesCategoryOverStanderd',
...
function exportExcelCos() {
	...
	exportExcelByCustom('report.PlantSituationService.exportCos','','searchConditionForm');
}
</script>
...
<!-- 							<a class="btn btngroup_usual" onclick="exportExcel('getVegetablesCategoryOverStanderd','','searchConditionForm')">导出Excel<i class="icon-share"></i></a> -->
							<a class="btn btngroup_usual" onclick="exportExcelCos()">导出Excel<i class="icon-share"></i></a>
						</div>
					</div>
					<table class="table table-striped table-bordered table-hover" id="standardForZZ_5_table" style="margin-top: 5px;">
						<thead>
							<tr>
								<th class="center hidden-480" >类别</th>
