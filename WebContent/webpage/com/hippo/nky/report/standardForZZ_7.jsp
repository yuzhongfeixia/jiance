...
<script>
var dataTable = null;
dataTable = $('#standardForZZ_7_table').dataTable({
	"bAutoWidth": true,
	"bPaginate" : false,
...
function drewTableCells(list){	
	...
	// 设置拼接的字符串到table中
	$("#standardForZZ_7_table").html(htmls);
...
$('#statisticsBtn').on('click', function(){
	...
	$.ajax({
		async: false,
		type : 'POST',
		url : 'plantSituationController.do?getVegetablesVarietyOverStanderd',
...
function exportExcel_vos(){
	...
	exportExcelByCustom('report.PlantSituationService.exportVos','','searchConditionForm')
}
</script>
...
<!-- 							<a class="btn btngroup_usual" onclick="exportExcel('getVegetablesVarietyOverStanderd','','searchConditionForm')">导出Excel<i class="icon-share"></i></a> -->
							<a class="btn btngroup_usual" onclick="exportExcel_vos()">导出Excel<i class="icon-share"></i></a>
						</div>
					</div>
					<table class="table table-striped table-bordered table-hover" id="standardForZZ_7_table" style="margin-top: 5px;">
						<thead>
							<tr>
								<th class="center hidden-480" >产品名称</th>
