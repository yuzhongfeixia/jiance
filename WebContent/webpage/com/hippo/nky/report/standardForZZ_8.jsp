...
<script>
var dataTable = null;
dataTable = $('#standardForZZ_8_table').dataTable({
	"bAutoWidth": true,
	"bPaginate" : false,
...
function drewTableCells(htmls){	
	...
	// 设置拼接的字符串到table中
	$("#standardForZZ_8_table").html(htmls);
...
$('#statisticsBtn').on('click', function(){
	...
	$.ajax({
		async: false,
		type : 'POST',
		url : 'plantSituationController.do?getCountiesOverStandardDetail',
...
function exportExcel_cosd(){
	...
	exportExcelByCustom('report.PlantSituationService.exportCosd','','searchConditionForm');
}
</script>
...
<!-- 										</select> -->
					...
					<div class="clearfix">
						<div class="pull-right">
							...
							<a class="btn btngroup_usual" onclick="exportExcel_cosd()">导出Excel<i class="icon-share"></i></a>
						</div>
					</div>
					<table class="table table-striped table-bordered table-hover" id="standardForZZ_8_table" style="margin-top: 5px;">
						<thead>
							<tr>
