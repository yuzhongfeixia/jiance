...
<script>
	$('#searchBtn').on('click', function(){
		...
		refresh_SamplingSetBack();
	});

  $(document).ready(function() {
	   registAjaxDataTable({
		   	id:"sample_obsolete_tb",
			actionUrl:"samplingInfoController.do?getSamplingSetBackCheck",	
			search:true,
	  ...
	  var idArr = "";
		...
		$("#confirmDiv").confirmModal({
			...
			callback: function () {
				$.ajax({
					type : "POST",
					url : "samplingInfoController.do?samplingSetBackCheckPast",
					data : {'idArr' : idArr},
	  ...
	  var idArr = "";
		...
		$("#confirmDiv").confirmModal({
			...
			callback: function () {
				$.ajax({
					type : "POST",
					url : "samplingInfoController.do?samplingSetBackChecksetBack",
					data : {'idArr' : idArr},
					success : function(data) {
						var d = $.parseJSON(data);
						if (d.success) {
  ...
  function refresh_SamplingSetBack(){
	  $("#sample_obsolete_tb").dataTable().fnPageChange('first');  
  }
