...
//  	            			   cq.eq(name, searchValue);
	...
	/**
  	 ...
  	 * @throws Exception
  	 */
  	public static void installHql(CriteriaQuery cq,Object searchObj, HttpServletRequest request){
  		PropertyDescriptor origDescriptors[] = PropertyUtils.getPropertyDescriptors(searchObj);
  		// 获得对象属性中的name 
  		String descriptorsNames = getDescriptorsNames(origDescriptors);
            ...
            if (PropertyUtils.isReadable(searchObj, name)) {
               if("class java.lang.String".equals(type)){
            	   ...
            	   if(null == request){
            		...
            		String beginDate = request.getParameter(name + "_begin");
            		String endDate = request.getParameter(name + "_end");
            		if(ConverterUtil.isNotEmpty(beginDate) && ConverterUtil.isEmpty(endDate)){
            			cq.eq(name, ConverterUtil.toDate(beginDate, "yyyy-MM-dd"));
            		} else if(ConverterUtil.isEmpty(beginDate) && ConverterUtil.isNotEmpty(endDate)){
            			cq.eq(name, ConverterUtil.toDate(endDate, "yyyy-MM-dd"));
            		} else {
                		if(ConverterUtil.isNotEmpty(beginDate)){
    						cq.ge(name, ConverterUtil.toDate(beginDate, "yyyy-MM-dd"));
                		}
                		if(ConverterUtil.isNotEmpty(endDate)){
    						cq.le(name, ConverterUtil.toDate(endDate, "yyyy-MM-dd"));
                		} 
            		}
//					Date value = (Date) PropertyUtils.getSimpleProperty(searchObj, name);
//					if (null != value) {
