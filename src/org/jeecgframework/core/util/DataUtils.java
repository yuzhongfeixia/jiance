...
public class DataUtils extends PropertyEditorSupport {
	 ...
	 * @return JSONObject
	 */
	public static JSONObject getJsonDatagridEasyUI(List<Map<String, Object>> mapList, int iTotalCnt,int sEcho, Db2Page[] dataExchanger) {
		//easyUI的dataGrid方式  －－－－这部分可以提取成统一处理
		String jsonTemp = "{\"sEcho\":" + sEcho + ",\'iTotalRecords\':" + iTotalCnt
				+ ",\"iTotalDisplayRecords\":" + iTotalCnt + ",\'aaData\':[";
		String tempStr = "";
		for (int j = 0; j < mapList.size(); j++) {
			...
			for (int i = 0; i < dataExchanger.length; i++) {
				...
				if(dataExchanger[i].getKey().equals("id")){
					tempStr = dataExchanger[i].getData(m).toString();
				}
	 ...
	 * @param iTotalCnt : 从数据库直接取得的结果集总数据条数
	 * @param dataExchanger : 页面表示数据与数据库字段的对应关系列表
	 * @param dataDicMap : 数据字典数据
	 * @return JSONObject
	 */
	public static JSONObject getJsonDatagridEasyUI(List<Map<String, Object>> mapList, int iTotalCnt,int sEcho, Db2Page[] dataExchanger, Map<String,String> dataDicMap) {
		//easyUI的dataGrid方式  －－－－这部分可以提取成统一处理
		String jsonTemp = "{\"sEcho\":" + sEcho + ",\'iTotalRecords\':" + iTotalCnt
				+ ",\"iTotalDisplayRecords\":" + iTotalCnt + ",\'aaData\':[";
		String tempStr = "";
		for (int j = 0; j < mapList.size(); j++) {
			...
			for (int i = 0; i < dataExchanger.length; i++) {
				...
				String dataDicName = dataDicMap.get(dataExchanger[i].getKey());
				if(ConverterUtil.isNotEmpty(dataDicName)){
					jsonTemp += ",'" + dataExchanger[i].getKey() + "_name\'" + ":'" + transform(dataDicName,objValue) + "'";
				}
			}
			jsonTemp += ",'DT_RowId':'"+tempStr+"'}";
			tempStr = "";
	...
	private static String transform(String dataDicName,Object value) { 
		...
		if(types != null){
			for (TSType type : types) {
				if (type.getTypecode().equals(ConverterUtil.toString(value))) {
	...
	public static String Str2Str(String date,String formatStr){
		if(date.length() > 19){
			date = date.substring(0, 19);
		}
		return Str2Str(date,"yyyy/MM/dd HH:mm:ss",formatStr);
	}
	
	public static String Str2Str(String date,String oldFormat,String newFormat){
		String newDate = "";
		try {
			newDate = formatDate(parseDate(date, oldFormat), newFormat);
		} catch (ParseException e) {
			e.printStackTrace();
		}
		return newDate;
