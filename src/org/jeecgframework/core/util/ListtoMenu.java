...
public class ListtoMenu {
	...
	public static String getEasyuiMultistageTree(
		...
		if(list!=null){
			for (TSFunction function : list) {
				if(siderbarHome.equals(function.getFunctionName())){
					menuString.append("<li class=\"root-menu start active \">");
					menuString.append("<a id=\"menu_" + function.getId() + "\" href=\"" + function.getFunctionUrl() + "\">");
					menuString.append("<i class=\"" + function.getIconId() + "\"></i>");
					menuString.append("<span class=\"title\">"+function.getFunctionName()+"</span>");
					menuString.append("<span class=\"selected\"></span>");
					menuString.append("</a>");
					menuString.append("</li>");
				} else {
					/*menuString.append("<div   title=\"" + function.getFunctionName()
					...
					menuString.append("<li class=\"root-menu\">");
					menuString.append("<a id=\"menu_" + function.getId() + "\" class=\"root-menu-text\" href=\"javascript:;\">");
					menuString.append("<i class=\"" + function.getIconId() + "\"></i>");
					menuString.append("<span class=\"title\">"+function.getFunctionName()+"</span>");
					menuString.append("<span class=\"arrow \"></span>");
					menuString.append("</a>");
		...
		for (TSFunction function : list) {
			if (function.getTSFunction().getId().equals(parent.getId())){
				if(function.getTSFunctions().size()==0||!map.containsKey(level+1)){
					...
					menuString.append("<a id=\"menu_" + function.getId() + "\" href=\"javascript:;\">");
					menuString.append("<i class=\"icon-upload-alt\"></i>");
					menuString.append("<span class=\"title\">"+function.getFunctionName()+"</span>");
					menuString.append("<span class=\"arrow \"></span>");
					menuString.append("</a>");
	...
	private static String getLeafOfTree(TSFunction function){
		...
		menuString.append("<li> ");
		menuString.append("<a id=\"menu_" + function.getId() + "\" class=\"ajaxify\" href=\""+function.getFunctionUrl()+"&clickFunctionId="+function.getId()+"\">");
		menuString.append(function.getFunctionName());
