...
public class DataGridTag extends TagSupport {
	 ...
	 * @param value
	 */
	public void setColumn(String title, String field, Integer width, String rowspan, String colspan, String align, boolean sortable, boolean checkbox, String formatter, boolean hidden, String replace, String treefield, boolean image, boolean query, String url, String funname, String arg,String queryMode, String dictionary,boolean frozenColumn,String extend,String style) {
...
//		sb.append("alert($(\".datagrid-footer\").find(\"td[field='opt']\").length());");
					...
					if (col.isQuery()) {
						...
						if("single".equals(col.getQueryMode())){
							if(!StringUtil.isEmpty(col.getReplace())){
								...
								sb.append("<input type=\"text\" name=\""+col.getField()+"\"  "+extendAttribute(col.getExtend())+" style=\"width: 100px\" />");
							}
		...
		if("group".equals(getQueryMode()) && hasQueryColum(columnList)){//如果表单是组合查询
			...
			for (DateGridColumn col : columnList) {
				if (col.isQuery()) {
					sb.append("<div data-options=\"name:\'"+col.getField()+"\',iconCls:\'icon-ok\' "+extendAttribute(col.getExtend())+" \">"+col.getTitle()+"</div>  ");
				}
			}
			sb.append("</div>");
			sb.append("</span>");
	...
	protected void getOptUrl(StringBuffer sb) {
		...
		for (DateGridUrl dateGridUrl : list) {
			...
			if (OptTypeDirection.OpenTab.equals(dateGridUrl.getType())) {
				sb.append("href+=\"[<a href=\'#\' onclick=addOneTab('" + dateGridUrl.getTitle() + "','" + url  + "')>\";");
			}
	 ...
	 * @version 1.0
	 */
	private void installOperationCode(DateGridUrl dateGridUrl,String operationCode,List optList){
		if(ResourceUtil.getSessionUserName().getUserName().equals("admin")|| !Globals.BUTTON_AUTHORITY_CHECK){
