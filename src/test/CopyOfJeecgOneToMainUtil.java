package test;

import java.util.ArrayList;
import java.util.List;

import org.jeecgframework.codegenerate.generate.onetomany.CodeGenerateOneToMany;
import org.jeecgframework.codegenerate.pojo.onetomany.CodeParamEntity;
import org.jeecgframework.codegenerate.pojo.onetomany.SubTableEntity;
import org.jeecgframework.codegenerate.util.def.JeecgKey;


/**
 * 代码生成器入口【一对多】
 *
 */
public class CopyOfJeecgOneToMainUtil {

	/**
	 * 一对多(父子表)数据模型，生成方法
	 * @param args
	 */
	public static void main(String[] args) {
		//第一步：设置主表
		CodeParamEntity codeParamEntityIn = new CodeParamEntity();
		codeParamEntityIn.setTableName("NKY_MONITORING_PLAN");//主表[表名]
		codeParamEntityIn.setEntityName("MonitoringPlan");	 //主表[实体名]
		codeParamEntityIn.setEntityPackage("test");	 //主表[包名]
		codeParamEntityIn.setFtlDescription("检测方案数据");	 //主表[描述]
		
		//第二步：设置子表集合
		List<SubTableEntity> subTabParamIn = new ArrayList<SubTableEntity>();
		//[1].子表一
		SubTableEntity po = new SubTableEntity();
		po.setTableName("nky_monitoring_project");//子表[表名]
		po.setEntityName("MonitoringProject");	 //子表[实体名]
		po.setEntityPackage("test");	 //子表[包]
		po.setFtlDescription("检测项目数据");		 //子表[描述]
		//子表[外键:与主表关联外键]
		//说明：这里面的外键是子表的外键字段,非主表和子表的对应关系
		//po.setForeignKeys(new String[]{"GORDER_ID","GO_ORDER_CODE"});
		po.setForeignKeys(new String[]{"PLAN_CODE"});
		subTabParamIn.add(po);
		
		//[2].子表二
		SubTableEntity po2 = new SubTableEntity();
		po2.setTableName("NKY_MONITORING_ATTACHMENT");		//子表[表名]
		po2.setEntityName("MonitoringAttachment");			//子表[实体名]
		po2.setEntityPackage("test"); 					//子表[包]
		po2.setFtlDescription("方案附件");				//子表[描述]
		//子表[外键:与主表关联外键]
		//说明：这里面的外键是子表的外键字段,非主表和子表的对应关系
		//po2.setForeignKeys(new String[]{"GORDER_ID","GO_ORDER_CODE"});
		po2.setForeignKeys(new String[]{"PLAN_CODE"});
		subTabParamIn.add(po2);
		
		codeParamEntityIn.setSubTabParam(subTabParamIn);
		//第三步：一对多(父子表)数据模型,代码生成
		CodeGenerateOneToMany.oneToManyCreate(subTabParamIn, codeParamEntityIn);
	}
}
